...
1. **Fetch README raw — pin to an immutable release tag, NOT mutable `main`** (mutable branch = instructions can change under you):
   - Resolve the latest release first: `https://github.com/UberMorgott/full-audit/releases/latest` redirects to the newest tag (currently `v1.14.1`).
...
> **Fetch pattern:** use the release tag resolved in step 1 (e.g. `v1.14.1`) for ALL subsequent file fetches: `https://raw.githubusercontent.com/{user}/full-audit/<release-tag>/{file}` — NOT mutable `main`. Keep `{user}` for forks.
