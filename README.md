...
1. **Fetch README** via WebFetch — pin to an immutable release tag, NOT mutable `main` (mutable branch = instructions can change under you):
   - Resolve the latest release first: `https://github.com/UberMorgott/full-audit/releases/latest` redirects to the newest tag (currently `v1.10.4`).
   - Then fetch raw at that exact resolved tag (e.g. `v1.10.4`): `https://raw.githubusercontent.com/UberMorgott/full-audit/<release-tag>/README.md`.
...
> **Fetch pattern:** use the release tag resolved in step 1 (e.g. `v1.10.4`) for ALL subsequent file fetches: `https://raw.githubusercontent.com/{user}/full-audit/<release-tag>/{file}` — NOT mutable `main`. Keep `{user}` for forks.
