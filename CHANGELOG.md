...
### Added
- **Meta-audit remediation — 29 audit-process defects fixed** (from a live L3 audit of an offline C# Unity/Harmony co-op mod: net472, single-DLL, Steam P2P, no web/DB/container). The generic recall-first engine fit well and found every real bug; the concrete checklist/tooling surface was ~70% web-service-oriented and misfired. Five fix classes, all additive:
  - **Class 1 — Spec misfit (stack-profile gating).** New **Stack Profile & Applicability Gating** contract (`universal.md`, `README.md` Phase 0, workflow `args.stack_profile`): a Phase-0 capability vector `has_http_surface`/`has_auth`/`has_db`/`has_container`/`has_package_manager`/`has_runtime_config`/`is_runnable` passed to every agent. Web-service sections (HTTP headers, CSRF, rate-limiting, XSS, SSRF, mass-assignment, session, JWT, API-contract, container, CI/CD, SBOM) and the csharp.md SQLi/EF/Blazor/ASP.NET tables carry an **Applicability** line — capability absent → clean documented SKIP (one-line limitation), NOT a finding/BLOCKER/re-review. SSRF scoped to user-URL-over-HTTP + a LAN carve-out; Config-Management split (magic-numbers/fail-fast universal, env-var/hot-reload/cliffs gated); Resilience reframed to the actual I/O boundary; Concurrency gains a single-threaded UI/game-loop + callback-marshalling profile; Purpose-Fit wording neutralized to "surface".
  - **Class 2 — Missing coverage (4 new L3 families + 2 folded checks).** New `universal.md` L3 sections: **Replicated/Authoritative State-Sync Correctness** (one-writer-per-field, mirror divergence, out-of-band mutation, reconvergence backstop + streamed-batch partial-apply), **Authenticated-Channel Trust Boundary** (peer-as-attacker; generalizes IDOR/BOLA off HTTP to any authenticated channel), **Runtime-Binding/Reflection Drift** (modding/plugin stacks), **Liveness & Recovery for Stateful Lifecycles** (bounded wait + recovery + notify-on-eviction). Untrusted length-prefix→allocation (CWE-770) folded into Deserialization Safety.
  - **Class 3 + 4 — Reproduction/confidence model for non-runnable, latent-defect codebases.** New `STATIC_CONFIRMED` verdict (structural claim verified by code read; floor 85, severity UNCHANGED — a runtime-latent HIGH is not demoted), `reproduced` admits `static-verified`, `repro_command` output repurposed as `verification_command`. `is_runnable=false` tells Wave 2.5 to verify statically instead of faking runtime reproduction (`REPRO_SCHEMA` + reproduction/verify prompts + scoring override, workflow `.js`; `WORKFLOW.md`, `README.md`).
  - **Class 4 — Debt-aware & decoupled severity.** Canon "violation = HIGH minimum" exempts roadmap-tracked spec-permitted debt; severity decoupled from reproduction-confidence; optional `related_invariant`/`design_ref` fields + formally-admitted `evidence_commits` (schema + `cleanFinding`). Split-test-suite coverage (pure vs runtime-gated) in `csharp.md` L1.
  - **Class 5 — Process contradictions.** Anti-Rationalization made stack-aware (gated-off section = clean skip, not a re-review trigger); trust-boundary downgrades (LAN/operator-local/offline) blessed as calibration; "tool not installed = BLOCKER" only when the tool is applicable to the stack; git-hygiene artifact list stack-pruned; git-wrapping-proxy (rtk) count-corruption noted.
- **Tooling (`tools.md`, `csharp.md`).** `requires: PackageReference/packages.config` on the NuGet-dependent C# tools with a local-DLL-no-package-manager downshift path; a PowerShell twin for the trufflehog install recipe.
